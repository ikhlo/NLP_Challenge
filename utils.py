...
def build_dataset(json_data, ecb_list, fed_list, labels=True):
  ...
  Args:
    (Dict) json_data: The JSON dict data.
    (List) ecb_list: All K-unique speeches from ECB bank.
    (List) fed_list: All L-unique speeches from FED bank.
    ...
    for day in range(T):
      for b in banks:
        ...
        if speech in eval(b.lower()+"_dict").keys():
          eval("idx_"+b.lower()).append(eval(b.lower()+"_dict")[speech])
        else:
          eval("idx_"+b.lower()).append(eval(b.lower()+"_dict")[''])
